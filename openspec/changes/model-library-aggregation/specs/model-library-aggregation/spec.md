...
定义模型库 **四表** 持久化模型、字段约束与实例选择语义；详细列定义以 **`design.md` 中 DDL 摘要** 为准。本规范不涵盖历史数据迁移。启用本能力时，**OpenAI 兼容 Chat / List models 与 `gateway.yaml` 中的 Upstream 配置不并存**，仅以四表及本文实例选择规则为唯一来源。
...
### Requirement: 健康检查（MAY）

系统 MAY 对 **`model_upstream`** 或实例做探针；失败时 MAY 临时排除实例且 MUST NOT 在日志中输出 **`api_key`** 原文。
