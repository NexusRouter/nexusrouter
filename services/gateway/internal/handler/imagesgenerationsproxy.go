...
const defaultImagesGenerationModel = "dall-e-2"

func mergeImagesGenerationsDefaultModel(body []byte) []byte {
	...
	b, err := json.Marshal(defaultImagesGenerationModel)
	if err != nil {
		return body
	}
...
func validateImagesGenerationsRequestBody(body []byte) error {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(body, &obj); err != nil {
	...
	raw, ok := obj["prompt"]
	if !ok {
		return errors.New("请求体缺少 prompt 字段")
	}
	if len(bytes.TrimSpace(raw)) == 0 || string(bytes.TrimSpace(raw)) == "null" {
		return errors.New("prompt 不可为 null 或空")
	}
	var s string
	if err := json.Unmarshal(raw, &s); err != nil {
		return errors.New("prompt 须为字符串")
	}
	if strings.TrimSpace(s) == "" {
		return errors.New("prompt 不可为空字符串")
	}
	return nil
}

// ImagesGenerationsProxy 将 POST /v1/images/generations 反向代理至上游；鉴权、限流、上游选择与出站头处理与 Moderations 代理对齐。
func ImagesGenerationsProxy(cfg *config.Config, log *zap.Logger, rt *runtime.Store, col *metrics.Collector, db *gorm.DB) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if err := validateImagesGenerationsRequestBody(body); err != nil {
			WriteGatewayError(c, http.StatusBadRequest, "INVALID_REQUEST", err.Error())
			return
		}
		body = mergeImagesGenerationsDefaultModel(body)
		...
		if db != nil && repository.UseDatabaseModelLibrary(db) {
			...
			if err != nil {
				if log != nil {
					log.Debug("images generations model pick failed", zap.String("model", modelName), zap.Error(err))
				}
		...
		rev.ErrorHandler = func(w http.ResponseWriter, r *http.Request, err error) {
			if log != nil {
				log.Warn("images generations proxy upstream error",
					zap.Error(err),
		...
		defer func() {
			if rec := recover(); rec != nil {
				if log != nil {
					log.Error("images generations proxy panic",
						zap.Any("error", rec),
