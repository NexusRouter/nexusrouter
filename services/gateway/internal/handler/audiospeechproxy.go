...
const defaultAudioSpeechModel = "tts-1"

func mergeAudioSpeechDefaultModel(body []byte) []byte {
	...
	b, err := json.Marshal(defaultAudioSpeechModel)
	if err != nil {
		return body
	}
...
func validateAudioSpeechRequestBody(body []byte) error {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(body, &obj); err != nil {
		return errors.New("请求体须为 JSON 对象")
	}
	if err := requireNonEmptyJSONString(obj, "input", "input"); err != nil {
		return err
	}
	if err := requireNonEmptyJSONString(obj, "voice", "voice"); err != nil {
		return err
	}
	return nil
}

func requireNonEmptyJSONString(obj map[string]json.RawMessage, key, label string) error {
	raw, ok := obj[key]
	if !ok {
		return errors.New("请求体缺少 " + label + " 字段")
	}
	if len(bytes.TrimSpace(raw)) == 0 || string(bytes.TrimSpace(raw)) == "null" {
		return errors.New(label + " 不可为 null 或空")
	}
	var s string
	if err := json.Unmarshal(raw, &s); err != nil {
		return errors.New(label + " 须为字符串")
	}
	if strings.TrimSpace(s) == "" {
		return errors.New(label + " 不可为空字符串")
	}
	return nil
}

// AudioSpeechProxy 将 POST /v1/audio/speech 反向代理至上游；鉴权、限流、上游选择与出站头处理与图像生成代理对齐。
func AudioSpeechProxy(cfg *config.Config, log *zap.Logger, rt *runtime.Store, col *metrics.Collector, db *gorm.DB) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if err := validateAudioSpeechRequestBody(body); err != nil {
			WriteGatewayError(c, http.StatusBadRequest, "INVALID_REQUEST", err.Error())
			return
		}
		body = mergeAudioSpeechDefaultModel(body)
		...
		if db != nil && repository.UseDatabaseModelLibrary(db) {
			...
			if err != nil {
				if log != nil {
					log.Debug("audio speech model pick failed", zap.String("model", modelName), zap.Error(err))
				}
		...
		rev.ErrorHandler = func(w http.ResponseWriter, r *http.Request, err error) {
			if log != nil {
				log.Warn("audio speech proxy upstream error",
					zap.Error(err),
		...
		defer func() {
			if rec := recover(); rec != nil {
				if log != nil {
					log.Error("audio speech proxy panic",
						zap.Any("error", rec),
